...
function invokeRenderer(renderFunc, dustChunk, dustContext, dustBodies, params, buildInput) {
    var attributes = dustContext.global.raptorAttributes;
    if (!attributes) {
        attributes = dustContext.get('raptorAttributes');
        dustContext.global.raptorAttributes = attributes = attributes || {};
    }
    ...
    renderContext.stream = dustContext.get('stream');
